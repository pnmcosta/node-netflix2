...
/**
 * Downloads the whole list of movies in mylist.
 * The Netflix endpoint is paged.
 * This structure is copied from getRatingHistory.
 * @param callback
 */
Netflix.prototype.getMyList = function (callback) {
  ...
  var page = 0
  var pages = 1
  var perPage = 48 // default from UI

  async.doWhilst(
    function (cb) {
      self.__getMyList(page, perPage, function (error, json) {
        if (error) {
          return callback(error)
        }
        page = page + 1
        pages = Math.floor(json.jsonGraph.mylist.length.value / perPage) + 1
        // TODO: normalize results?
        listItems = listItems.concat(json.videos)
        cb(null)
      })
    },
    function () {
    ...
    function (error) {
      callback(error, listItems)
    }
  )
}

Netflix.prototype.__getMyList = function (page, perPage, callback) {
  var pager = {
    from: page * perPage,
    to: (page + 1) * perPage
  }

  var defaultQuery = ['mylist']

  // TODO: assert whether all these qs's/path's are all actually needed
  var options = {
    method: 'POST',
    qs: {
      drmSystem: 'widevine',
    ...
    body: {
      path: [
        [...defaultQuery, ['id', 'length', 'listId', 'name', 'requestId', 'trackIds']],
        [...defaultQuery, pager, ['promoVideo', 'summary', 'title', 'titleMaturity', 'userRating', 'userRatingRequestId']],
        [...defaultQuery, pager, 'boxarts', '_342x192', 'webp']
      ]
    }
  }
